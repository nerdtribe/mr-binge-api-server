...
let getDetailedMovieSearchURL = (id) => {
    return `${baseURL}movie/${id}?api_key=${process.env.TMDB_API_KEY}&language=en-US&page=1&include_adult=false&append_to_response=videos`;
};

// // Function that returns the URL for specific TV show searches
let getDetailedTVSearchURL = (id) => {
    return `${baseURL}tv/${id}?api_key=${process.env.TMDB_API_KEY}&language=en-US&page=1&include_adult=false&append_to_response=videos`;
};
